...
export function documentsBrowserHandler(req: Request, res: Response, next: NextFunction) {
  ...
  if (req.method === 'GET' && isDocumentsPath && isBrowserRequest(req)) {
    log(`[SdegdaefkHandler] Browser request detected for documents: ${req.path}`, 'info');
    ...
    try {
      ...
      log(`[SdegdaefkHandler] Redirecting browser request for documents: ${req.path}`, 'info');
      ...
      log(`[SdegdaefkHandler] Error handling browser request: ${error.message}`, 'error');
      return next(error);
    }
  }
