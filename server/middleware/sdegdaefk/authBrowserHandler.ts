...
export function authBrowserHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Log all requests to /auth for debugging
    if (isAuthPath) {
      log(`[SdegdaefkHandler] Received request for auth path: ${req.path}`, 'info');
    ...
    log(`[SdegdaefkHandler] Error handling auth request: ${error.message}`, 'error');
    ...
    const errorHtml = `
      ...
      <html>
        ...
        <body>
          ...
          <p><span class="error-code">Κωδικός: AUTH-ERR-500</span></p>
        </body>
      </html>
    `;
