...
export const selectBeneficiaryPaymentSchema = createSelectSchema(beneficiaryPayments);
    ...
    switch (tableName) {
      case 'projects':
        ...
        if (!data.event_description) {
          errors.push({ field: 'event_description', message: 'Event description is required' });
        }
        break;
        
      case 'beneficiary_payments':
        if (!data.beneficiary_id) {
          errors.push({ field: 'beneficiary_id', message: 'Beneficiary ID is required' });
        }
      ...
      case 'generated_documents':
        if (!data.generated_by) {
          errors.push({ field: 'generated_by', message: 'Generated by user ID is required' });
        }
        break;
      ...
      case 'budget_history':
        ...
        if (!data.change_type) {
          errors.push({ field: 'change_type', message: 'Change type is required' });
        }
        break;
    }
