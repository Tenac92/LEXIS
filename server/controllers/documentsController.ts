...
// Debug endpoint for systematic document corruption debugging
router.get("/debug-sections/:docId", authenticateSession, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const docId = parseInt(req.params.docId);
    const { 
      disablePaymentTable = false, 
      disableFooter = false, 
      disableHeader = false, 
    ...
    console.log(`[DEBUG] Testing document ${docId} with options:`, {
      disablePaymentTable, disableFooter, disableHeader, disableSubject, logTableCells
    });

    // Get document with enhanced data
    const { data: document, error: docError } = await supabase
      .from("generated_documents")
      .select(`
        *,
        generated_by:generated_by_id(name, department, telephone)
      `)
      .eq("id", docId)
      .single();

    if (docError || !document) {
      return res.status(404).json({ error: "Document not found", details: docError });
    }

    // Get project data - matching the export endpoint logic
    let projectData = null;
    if (document.project_index_id) {
      const { data: indexData, error: indexError } = await supabase
        .from("project_index")
        .select("project_id, expenditure_type_id")
        .eq("id", document.project_index_id)
        .single();

      if (!indexError && indexData) {
        const [projectRes, expenditureRes] = await Promise.all([
          supabase.from("Projects").select("*").eq("id", indexData.project_id).single(),
          supabase.from("expenditure_types").select("*").eq("id", indexData.expenditure_type_id).single()
        ]);

        if (!projectRes.error && !expenditureRes.error) {
          projectData = projectRes.data;
          document.expenditure_type = expenditureRes.data?.expenditure_types || "Unknown";
        }
      }
    }
    ...
    // Prepare document data - matching export endpoint format
    const documentData = {
      ...document,
      ...
      unit: document.unit_id?.toString() || "2",
    ...
    const debugOptions = {
      disablePaymentTable: disablePaymentTable === 'true',
      disableFooter: disableFooter === 'true',
      disableHeader: disableHeader === 'true',
      disableSubject: disableSubject === 'true',
      logTableCells: logTableCells === 'true'
    };

    console.log(`[DEBUG] Calling DocumentGenerator with debug options:`, debugOptions);
    const primaryBuffer = await DocumentGenerator.generatePrimaryDocument(documentData, debugOptions);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=debug-document-${docId}-sections.docx`,
    );
    res.send(primaryBuffer);

    console.log(`[DEBUG] Document ${docId} generated successfully with debug options`);
  } catch (error) {
    console.error("[DEBUG] Section debugging error:", error);
    res.status(500).json({
      error: "Failed to generate debug document",
