...
router.put('/:mis/formulations', authenticateSession, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { mis } = req.params;
    const { formulation_details, budget_versions, location_details } = req.body;
    ...
    if (formulation_details && Array.isArray(formulation_details) && formulation_details.length > 0) {
          ...
          if (budgetVersions.pde && Array.isArray(budgetVersions.pde)) {
            budgetVersions.pde.forEach((pdeVersion: any) => {
              budgetVersionsToInsert.push({
                ...
                boundary_budget: parseEuropeanBudget(pdeVersion.boundary_budget || pdeVersion.project_budget),
                protocol_number: pdeVersion.protocol_number || null,
                ada: pdeVersion.ada || null,
                decision_date: pdeVersion.decision_date || null,
                action_type: pdeVersion.action_type || pdeVersion.decision_type || 'Έγκριση',
                comments: pdeVersion.comments || null,
                created_by: req.user?.id || null,
                updated_by: req.user?.id || null
              });
          ...
          if (budgetVersions.epa && Array.isArray(budgetVersions.epa)) {
            budgetVersions.epa.forEach((epaVersion: any) => {
              budgetVersionsToInsert.push({
                ...
                epa_version: epaVersion.epa_version || null,
                boundary_budget: parseEuropeanBudget(epaVersion.boundary_budget || epaVersion.amount),
                protocol_number: epaVersion.protocol_number || null,
                ada: epaVersion.ada || null,
                decision_date: epaVersion.decision_date || null,
                action_type: epaVersion.action_type || epaVersion.decision_type || 'Έγκριση',
                comments: epaVersion.comments || null,
                created_by: req.user?.id || null,
                updated_by: req.user?.id || null
              });
      ...
      if (location_details && Array.isArray(location_details) && location_details.length > 0) {
        console.log(`[ProjectFormulations] Processing ${location_details.length} location details`);
        ...
        if (deleteIndexError) {
          console.error(`[ProjectFormulations] Error deleting existing project_index:`, deleteIndexError);
        } else {
          console.log(`[ProjectFormulations] Deleted existing project_index entries`);
        }

        // Get reference data for lookups
        const [eventTypesRes, expenditureTypesRes, monadaRes] = await Promise.all([
          supabase.from('event_types').select('*'),
          supabase.from('expenditure_types').select('*'),
          supabase.from('Monada').select('*')
        ]);
        ...
        for (const locationDetail of location_details) {
          try {
            ...
            if (locationDetail.event_type) {
              const eventType = eventTypes.find(et => et.name === locationDetail.event_type);
              eventTypeId = eventType?.id || null;
              console.log(`[ProjectFormulations] Event type "${locationDetail.event_type}" -> ID: ${eventTypeId}`);
            }
            ...
            if (locationDetail.implementing_agency) {
              const monada = monadaData.find(m => {
                const unitName = (typeof m.unit_name === 'object' && m.unit_name.name) ? m.unit_name.name : m.unit_name;
                return unitName === locationDetail.implementing_agency || m.unit === locationDetail.implementing_agency;
              });
              monadaId = monada?.id || null;
              console.log(`[ProjectFormulations] Agency "${locationDetail.implementing_agency}" -> ID: ${monadaId}`);
            }
            ...
            for (const expType of expenditureTypesToProcess) {
              const expenditureType = expenditureTypes.find(et => et.expenditure_types === expType);
              ...
              if (expenditureTypeId && eventTypeId && monadaId) {
                ...
                for (const geographicArea of geographicAreas) {
                  ...
                  console.log(`[ProjectFormulations] Processing geographic area:`, {
                    region: regionName,
                  ...
                  if (insertError) {
                    console.error(`[ProjectFormulations] Error inserting project_index:`, insertError);
                  } else {
                    console.log(`[ProjectFormulations] Created project_index entry ID: ${insertedEntry.id}`);
                    ...
                    if (regionName || regionalUnitName || municipalityName) {
                      ...
                      // Helper function to insert geographic relationships
                      const insertGeographicRelationships = async (projectIndexId: number, region: any) => {
                        const relationships = [];
                        ...
                        if (relationships.length > 0) {
                          ...
                          console.log(`[ProjectFormulations] Created ${relationships.length} geographic relationships for project_index ${projectIndexId}`);
                        }
                      };
                      
                      await insertGeographicRelationships(insertedEntry.id, regionObject);
                    }
                  }
                }
              } else {
                console.log(`[ProjectFormulations] Skipping entry due to missing IDs:`, {
                  expenditureTypeId,
                  eventTypeId, 
                  monadaId
                });
            ...
            console.error(`[ProjectFormulations] Error processing location detail:`, detailError);
          }
        }
        
        console.log(`[ProjectFormulations] Completed processing all location details`);
      }
...
router.put('/:mis/comprehensive', authenticateSession, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (formData.location_details && Array.isArray(formData.location_details)) {
      console.log(`[ComprehensiveUpdate] Processing ${formData.location_details.length} location details`);
      ...
      if (deleteIndexError) {
        console.error(`[ComprehensiveUpdate] Error deleting existing project_index:`, deleteIndexError);
      } else {
        console.log(`[ComprehensiveUpdate] Deleted existing project_index entries`);
      }
      ...
      if (projectIndexIdsToDelete.data && projectIndexIdsToDelete.data.length > 0) {
        const indexIds = projectIndexIdsToDelete.data.map(idx => idx.id);
        await Promise.all([
          supabase.from('project_index_regions').delete().in('project_index_id', indexIds),
          supabase.from('project_index_units').delete().in('project_index_id', indexIds),
          supabase.from('project_index_munis').delete().in('project_index_id', indexIds)
        ]);
        console.log(`[ComprehensiveUpdate] Deleted existing geographic relationships`);
      }
      ...
      for (const locationDetail of formData.location_details) {
        try {
          ...
          if (locationDetail.event_type) {
            const eventType = eventTypes.find(et => et.name === locationDetail.event_type);
            eventTypeId = eventType?.id || null;
            console.log(`[ComprehensiveUpdate] Event type "${locationDetail.event_type}" -> ID: ${eventTypeId}`);
          }
          ...
          if (locationDetail.implementing_agency) {
            ...
            monadaId = monada?.id || null;
            console.log(`[ComprehensiveUpdate] Agency "${locationDetail.implementing_agency}" -> ID: ${monadaId}`);
          }
          ...
          for (const expType of expenditureTypesToProcess) {
            ...
            if (expenditureTypeId && eventTypeId && monadaId) {
              ...
              for (const geographicArea of geographicAreas) {
                ...
                console.log(`[ComprehensiveUpdate] Processing geographic area:`, {
                  region: regionName,
                  regionalUnit: regionalUnitName,
                  municipality: municipalityName
                });
                ...
                if (insertError) {
                  console.error(`[ComprehensiveUpdate] Error inserting project_index:`, insertError);
                } else {
                  console.log(`[ComprehensiveUpdate] Created project_index entry ID: ${insertedEntry.id}`);
              ...
              console.warn(`[ComprehensiveUpdate] Missing required IDs: expenditure=${expenditureTypeId}, event=${eventTypeId}, monada=${monadaId}`);
            }
          }
        } catch (locationError) {
      ...
      console.log(`[ComprehensiveUpdate] Completed processing location details`);
    }
