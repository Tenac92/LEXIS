...
// PERFORMANCE OPTIMIZATION: Batch save endpoint for comprehensive project updates
router.patch('/:mis/batch-save', authenticateSession, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { mis } = req.params;
    const batchData = req.body;
    console.log(`[ProjectBatchSave] Starting batch save for MIS: ${mis}`);
    ...
    const results: { 
      ...
      formulations: any[]; 
      errors: string[] 
    } = { project: null, decisions: [], formulations: [], errors: [] };

    // 2. Update project data if provided
    if (batchData.projectUpdate) {
      try {
        const { data: updatedProject, error: updateError } = await supabase
          .from('Projects')
          .update({
            ...batchData.projectUpdate,
        ...
        console.log(`[ProjectBatchSave] ✓ Project updated`);
      } catch (error: any) {
        console.error(`[ProjectBatchSave] ✗ Project update failed:`, error);
        results.errors.push(`Project update: ${error?.message || error}`);
      }
    }

    // 3. Batch process decisions if provided
    if (batchData.decisions && Array.isArray(batchData.decisions)) {
      try {
        ...
        if (batchData.decisions.length > 0) {
          const decisionsToInsert = batchData.decisions.map((decision: any, index: number) => ({
            ...
            decision_budget: decision.decision_budget || null,
            expenditure_type: decision.expenditure_type || [],
            decision_date: decision.decision_date || new Date().toISOString().split('T')[0],
          ...
          results.decisions = createdDecisions || [];
        }
        console.log(`[ProjectBatchSave] ✓ ${batchData.decisions.length} decisions processed`);
      } catch (error: any) {
        console.error(`[ProjectBatchSave] ✗ Decisions batch failed:`, error);
        results.errors.push(`Decisions: ${error?.message || error}`);
      }
    }

    // 4. Batch process formulations if provided
    if (batchData.formulations && Array.isArray(batchData.formulations)) {
      try {
        ...
        // Batch insert new formulations
        if (batchData.formulations.length > 0) {
          const formulationsToInsert = batchData.formulations.map((formulation: any, index: number) => ({
            project_id: projectId,
            ...
            project_budget: formulation.project_budget || null,
            total_public_expense: formulation.total_public_expense || null,
            eligible_public_expense: formulation.eligible_public_expense || null,
            ...
            connected_decision_ids: formulation.connected_decisions || [],
            comments: formulation.comments || null,
            created_by: req.user!.id,
            updated_by: req.user!.id
          }));

          const { data: createdFormulations, error: formulationsError } = await supabase
            .from('project_formulations')
            .insert(formulationsToInsert)
            .select();

          if (formulationsError) throw formulationsError;
          results.formulations = createdFormulations || [];
        }
        console.log(`[ProjectBatchSave] ✓ ${batchData.formulations.length} formulations processed`);
      } catch (error: any) {
        console.error(`[ProjectBatchSave] ✗ Formulations batch failed:`, error);
        results.errors.push(`Formulations: ${error?.message || error}`);
      }
    }
    ...
    console.log(`[ProjectBatchSave] Batch save completed in ${totalTime}ms`);
    console.log(`[ProjectBatchSave] Results: ${results.errors.length} errors, project: ${!!results.project}, decisions: ${results.decisions.length}, formulations: ${results.formulations.length}`);

    // Return results with performance metrics
    res.json({
      success: results.errors.length === 0,
      performance: { totalTime, timestamp: new Date().toISOString() },
      results,
      message: results.errors.length === 0 ? 'Batch save completed successfully' : 'Batch save completed with some errors'
    });

  } catch (error) {
    console.error('[ProjectBatchSave] Unexpected error:', error);
    res.status(500).json({ 
