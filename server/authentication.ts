...
export const authenticateSession = async (
  ...
  try {
    ...
    if (!req.user) {
      console.error(
        "[Auth] Failed to set req.user despite having session user",
      );
...
export function authenticateToken(
  ...
  try {
    ...
    if (
      sessionCreatedAt > 0 &&
      currentTime - sessionCreatedAt > MAX_SESSION_AGE
    ) {
      console.log("[Auth] Session age check:", {
        age:
          Math.floor((currentTime - sessionCreatedAt) / (60 * 60 * 1000)) +
          " hours",
...
export async function authenticateUser(
  ...
  try {
    ...
    if (userError) {
      console.error(
        `[Auth] Database error when retrieving user: ${email}`,
        userError,
      );
      return null;
    }
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        console.log(
          "[Auth] Password changed successfully for user:",
          req.user.id,
        );
