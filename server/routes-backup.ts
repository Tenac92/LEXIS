...
export async function registerRoutes(app: Express): Promise<Server> {
  try {
    ...
    // VERSION 2 DOCUMENT CREATION ENDPOINT - REMOVED - Now handled by DocumentsController
    // The v2-documents endpoint has been moved to server/controllers/documentsController.ts
    ...
    // Budget lookup moved to budget router to avoid conflicts with /history routes
    ...
    // NOTE: User documents endpoint moved to documentsController.ts
    // app.get('/api/documents/user') - now handled by documentsRouter
