-- Linear Project History Table Schema Migration for Supabase (FIXED)
-- 
...
-- Step 6: Helper function to safely extract from JSONB arrays
CREATE OR REPLACE FUNCTION safe_jsonb_extract_text(jsonb_data JSONB, key TEXT, index_pos INTEGER DEFAULT 0)
RETURNS TEXT AS $$
BEGIN
  -- Check if the key exists and is an array
  IF jsonb_data ? key AND jsonb_typeof(jsonb_data->key) = 'array' THEN
    -- Check if array has elements at the requested index
    IF jsonb_array_length(jsonb_data->key) > index_pos THEN
      RETURN jsonb_data->key->>index_pos;
    END IF;
  ELSIF jsonb_data ? key AND jsonb_typeof(jsonb_data->key) = 'string' THEN
...
-- Step 7: Migrate data from backup table to new linear structure
...
SELECT 
  ...
  -- Extract document references using safe function
  safe_jsonb_extract_text(decisions, 'kya', 0) as protocol_number,
  safe_jsonb_extract_text(decisions, 'fek', 0) as fek,
  safe_jsonb_extract_text(decisions, 'ada', 0) as ada,
  safe_jsonb_extract_text(decisions, 'budget_decision', 0) as budget_decision,
  safe_jsonb_extract_text(decisions, 'funding_decision', 0) as funding_decision,
  safe_jsonb_extract_text(decisions, 'ada_import_sana271', 0) as ada_import_sana271,
  safe_jsonb_extract_text(decisions, 'ada_import_sana853', 0) as ada_import_sana853,
  safe_jsonb_extract_text(decisions, 'allocation_decision', 0) as allocation_decision,
...
-- Step 8: Drop the helper function (cleanup)
DROP FUNCTION safe_jsonb_extract_text(JSONB, TEXT, INTEGER);
...
-- Step 10: Show sample linear data
SELECT 
  id,
  project_id,
  change_type,
