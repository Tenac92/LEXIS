...
export function getGeographicCodeForSave(location: {
  ...
  const hasRegion = location.region && location.region !== "__clear__";
  const hasRegionalUnit = location.regional_unit && location.regional_unit !== "__clear__";
  const hasMunicipality = location.municipality && location.municipality !== "__clear__";
